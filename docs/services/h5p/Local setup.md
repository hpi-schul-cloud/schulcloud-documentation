...
## Introduction
This documentation is to help developers to setup the servers and dependencies required to implement and test h5p in boards.
...
[minio](https://min.io/docs/minio/container/index.html) will be run in docker for storing h5p contents and libraries locally. 
...
If not yet done, create an access key pair. Make sure to store the access key secret well, as you cannot re-read it 
after creation.
...
You may also use any other names you prefer or even just use one bucket. If you do, make sure you adapt to the rest of 
the steps correctly.

This is optional, you can also change the s3 region to **eu-central-1**. If you decided not to change the region, please 
note that the default s3 region for minio is **us-east-1**.
...
You may run into problems while running the app, the [troubleshooting](#troubleshooting-setup-problems) below contains 
information to help you solve them.

If you remove any of the folders in the bucket, remember to remove the corresponding metadata from mongodb in the 
...
If the port 8080 is already in used, change the first "8080" to another valid port number. Do take note of this port number; 
it is important.
...
2. Create a course & board
3. Create a h5p element
4. Click on the empty h5p element

If everything works you should see a new window appear with the h5p-editor. The editor should contain the library you 
...
This section lists several problems that may occur when setting up h5p locally and possible solutions to them.
...
Check if the ports in your docker command/compose file are correct. If you change the 9001 port to another port number, 
...
If you are using docker in WSL, the problem could be that your apps (i.e. file-storage, h5p-library-management) than are run 
...
A quick workaround for this is by replacing "localhost" in the H5P_EDITOR__S3_ENDPOINT with the aforementioned exact ip address. 
...
You may be presented with multiple ip addresses. If this is the case, then you will need to find the correct one via try-and-error 
(replacing **localhost** with each address and re-rerunning the app until no error is thrown). This is just an assumption, 
but the correct ip address seems to not end with XXX.XX.0.1.

Another workaround is to run your apps in the wsl. The connection is then run in the wsl network, removing the need to resolve 
**localhost** between the windows and wsl network.

The real solution to this is probably to configure the dns resolution between wsl and windows correctly. This is 
